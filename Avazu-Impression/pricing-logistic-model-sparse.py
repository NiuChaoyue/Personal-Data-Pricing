...
@Version: The pure version (Sparse Case)
@Author: Chaoyue Niu
...
def Read_CTR_market_values(T, n):
    fy = open("../new-input/train_Y_n%d"%n)
    y = np.zeros(T, float)
...
def Read_CTR_market_value_thetastar(n):
    fw = open("../new-input/theta_n%d"%n)
...
if __name__=="__main__":
    ...
    """
    y = Read_CTR_market_values(T, n)

    """
    Read trained (not true) weight vector in market value from file
    """
    thetaStar = Read_CTR_market_value_thetastar(n)
    ...
    """
    fX = open("../new-input/train_X_n%d" % n)
    ...
    for t in range(T):
        ...
        print("Regret: %f\n"%(regretVec[t]))
    fX.close()
    np.savetxt("./regret-total/RegretTotal_RP_rpratio%f_n%d_T_%d" % (rp_ratio, n, T), totalregretVec, fmt='%.10f')
    ...
    np.savetxt("./regret-ratio/RegretRatio_RP_rpratio%f_n%d_T_%d" % (rp_ratio, n, T), regretRatio, fmt='%.10f')