...
@Version: The version with both reserve price and uncertainty (Multi-Dimensional)
@Author: Chaoyue Niu
...
if __name__=="__main__":
    ...
    np.savetxt("./regret-total/TotalRegrets_RP_Noise_n%d_T_%d" % (n, T), totalregretVec, fmt='%.10f')
    #np.savetxt("./regret/Regrets_RP_Noise_n%d_T_%d"%(n,T), regretVec, fmt='%.10f')
    # save regret ratio to file
    for t in range(T):
        if (totalMarketValue[t] != 0):
            regretRatio[t] = totalregretVec[t] * 1.0 / totalMarketValue[t]
    np.savetxt("./regret-ratio/RegretRatio_RP_Noise_n%d_T_%d" % (n, T), regretRatio, fmt='%.10f')