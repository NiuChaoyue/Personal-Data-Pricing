"""
@Topic: Pricing of Accommodation Rental Under Log Linear Market Value Model
@Version: Pure version && The versions with different reserve price parameters
@Author: Chaoyue Niu
...
def Read_Rental_feature_vector(city, T, n):
    fX = open("../new-input/%s_full_X"%city)
...
def Read_Rental_market_values(city, T, n):
    fy = open("../new-input/%s_full_y"%city)
    y = np.zeros(T, float)
...
def Read_Rental_market_value_thetastar(city, T, n):
    fw = open("../new-input/%s_w"%city)
...
if __name__=="__main__":
    ...
    """
    Read Feature Vectors and Market Values from File
    """
    X = Read_Rental_feature_vector(city, T, n)
    y = Read_Rental_market_values(city, T, n)

    """
    Read trained (not true) weight vector in market value from file
    """
    thetaStar = Read_Rental_market_value_thetastar(city, T, n)

    """
    log ratio between market value and reserve price
    """
    logratio = 0.0
    ...
    for t in range(T):
        ...
        if(qt >= (pt_upper + delta)):
            if (t == 0):
                ...
                totalMarketValue[t] = math.exp(vt)
            else:
                totalregretVec[t] = totalregretVec[t - 1] + regretVec[t]
                totalMarketValue[t] = totalMarketValue[t - 1] + math.exp(vt)
            continue
        else:
            ...
            #conservative posted price
                ...
                if (pt > vt):
                    if ((vt >= qt) and (pt - vt > 0.01)):
                        regretVec[t] = math.exp(vt)
                else:
                    regretVec[t] = math.exp(vt) - math.exp(pt)
                if (t == 0):
                    totalregretVec[t] = regretVec[t]
                    totalMarketValue[t] = math.exp(vt)
                else:
                    totalregretVec[t] = totalregretVec[t - 1] + regretVec[t]
                    totalMarketValue[t] = totalMarketValue[t - 1] + math.exp(vt)
                # don't use conservative price to update the ellipsoid
                continue
                #pt = pt_lower
        ...
        print("Regret: %f\n"%(regretVec[t]))
    np.savetxt("./regret-total/%s-RegretTotal_logratio_%f_n%d_T_%d" % (city, logratio, n, T), totalregretVec, fmt='%.10f')
    ...
    np.savetxt("./regret-ratio/%s-RegretRatio_logratio_%f_n%d_T_%d" % (city, logratio, n, T), regretRatio, fmt='%.10f')